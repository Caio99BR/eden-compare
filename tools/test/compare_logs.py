...
missing_modules = [m for m in required_modules if not __import__(m, globals(), locals(), [], 0)]
if missing_modules:
...
try:
    ...
    HAS_PYPLOT = False
    print("[compare_logs.py] 'matplotlib.pyplot' not found. Plotting will be skipped.")
...
def process_game_folder(game_folder):
    ...
    for column_name, metric_label, is_fps in metrics:
        ...
        for d in data:
            build_key = d['build_folder']
            display_label = build_key + "_master" if "_master" in build_key else build_key  # só para exibir

            plot_metric(
                d['df'], column_name, build_colors, build_key, d['relative_path'],
                d['version_short'], summary_file=d['summary_file'] if is_fps else None,
                is_fps=is_fps, ax=plt.gca()
            )
    ...
    if HAS_PYPLOT:
        ...
        summary_texts = {m[0]: [] for m in metrics}
        for d in data:
            for idx, (col, _, is_fps) in enumerate(metrics):
                build_key = d['build_folder']
                display_label = build_key + "_master" if "_master" in build_key else build_key

                text = plot_metric(
                    d['df'], col, build_colors, build_key, d['relative_path'],
                    d['version_short'], summary_file=d['summary_file'] if is_fps else None,
                    is_fps=is_fps, ax=axs[idx]
                )
                if text:
                    summary_texts[col].append(text)
